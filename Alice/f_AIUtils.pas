...
Var
  ...
  Preprocessor:TAISimpleSubstituter;
implementation
  Uses SysUtils;
  procedure WrFile(fname:string;s:string);
    var
  ...
  constructor TAIStringTokenizer.create(delimiter:string);
    begin
      inherited create;
      _delim:=delimiter;
  ...
  destructor TAIStringTokenizer.Destroy;
    begin
    _Tokens.Free;
    inherited;
  end;

  procedure TAIStringTokenizer.Tokenize(s:string);
    var
      i:integer;
      thistoken:string;
      spos,epos:integer;
...
procedure TAIStringTokenizer.SetDelimiter(delimiter:string);
 begin
...
function TAIStringTokenizer.getLast:string;
    begin
...
function TAIStringTokenizer.getFirst:string;
begin
      if _Tokens.count>0 then
...
function TAIStringTokenizer.get(i:integer):string;
begin
...
constructor TAISimpleSubstituter.create;
begin
      inherited create;
...
destructor TAISimpleSubstituter.destroy;
    begin
      _substFrom.free;
      _substTo.free;
      inherited destroy;
  ...
  procedure TAISimpleSubstituter.add(_from,_to:string);
    begin
      _substFrom.Add(_from);
      _substTo.Add(_to);
    end;
  function TAISimpleSubstituter.process(s:string):string;
    var
      i:integer;
    begin
      result:=s;
