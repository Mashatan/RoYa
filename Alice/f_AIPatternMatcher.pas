...
unit f_AIPatternMatcher;

interface
uses classes,f_AIUtils;
const
...
{TMatch stores information about a match, such as matched wildcards,
 or the resulting template}
  ...
  TPatternMatcher=class
    _root:TPatternNode;
    _tokenizer:TAIStringTokenizer;
    _SentenceTokenizer:TAIStringTokenizer;
    _count:integer;
    _locked:boolean;
    _matchfault:integer;
...
implementation
  ...
  var
    j,i:Integer;
  begin
    for i:= 0 to CNumContext-1 do begin
      if _childs[i]<>nil then
        FreeAndNil(_childs[i]);
    end;
    FreeMemory(_childs);
    FreeMemory(_count);
    inherited;
...
*)
  ...
  begin
    if i<=_count[context] then begin
      ...
      _childs[context].Objects[i].Free; // Added by Ali Mashatan 2008-10-15
  ...
  begin
    ...
    _tokenizer:=TAIStringTokenizer.create(' ');
  ...
  begin
    ...
    if not assigned(_SentenceTokenizer) then
      _SentenceTokenizer:=TAIStringTokenizer.Create(f_AIUtils.SentenceSplitterChars);
